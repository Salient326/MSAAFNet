...
model = MSAAFNet()
model.load_state_dict(torch.load('./models/MSAAFNet/MSAAFNet_DUTS-TR.pth'))

model.cuda()
model.eval()

test_datasets = ['ECSSD']   # 'ECSSD', 'PASCALS', 'DUT-O', 'HKU-IS', 'DUTS-TE', 'ORSI-4199', 'EORSSD', 'ORSSD'

for dataset in test_datasets:
    save_path = './results/' + 'MSAAFNet-' + dataset + '/'
